...
public class ShopController {
    ...
    @PutMapping("/open-request/{shopId}/approve")
    @Operation(
            summary = "쇼핑몰 개설 요청 허가",
            description = "<p>관리자는 개설 신청된 쇼핑몰의 목록을 확인하고 정보를 확인한 후 신청을 허가할 수 있다.</p>"
    )
    public ResponseEntity<ShopDto> openRequestApprove(
            @PathVariable(value = "shopId")
            Long shopId
    ) {
        return ResponseEntity.ok(shopService.openRequestApprove(shopId));
    }

    @PutMapping("/open-request/{shopId}/reject")
    @Operation(
    ...
    @PutMapping("/close-request/{shopId}/approve")
    @Operation(
            summary = "쇼핑몰 폐쇄 요청 허가",
            description = "<p>관리자는 쇼핑몰 페쇄 요청 목록을 확인한 후 요청을 허가할 수 있다.</p>"
    )
    public ResponseEntity<ShopDto> closeRequestApprove(
            @PathVariable(value = "shopId")
            Long shopId
    ) {
        return ResponseEntity.ok(shopService.closeRequestApprove(shopId));
    }
